...
import { addTodo, clearCompletedTodos, getPaginatedTodos, getFilteredTodos } from '../../services/todos'; // Importa las funciones de servicio para manejar las tareas
...
export const GET: APIRoute = async ({ request, redirect }) => {
  ...
  try {
    ...
    const paginatedTodos = getPaginatedTodos(filter, validPage, validLimit); // Se obtienen las tareas filtradas según el estado actual del filtro
    ...
    if (contentType === "application/json") {
      ...
      return new Response("Redirecting...", { status: 302, headers: { Location: `/?filter=${filter}&page=${validPage}&limit=${validPage}` } });
    }
  } catch (error) {
