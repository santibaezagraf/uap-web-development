...
import { addTodo, toggleTodo, deleteTodo, clearCompletedTodos, getFilteredTodos } from '../../state';
...
const parseFormData = async (request: Request) => {
    ...
    return { action, text, id, filter };	
};

const parseJson = async (
  request: Request
  ): Promise<{ action: string | undefined; text: string | undefined; id: string | undefined; filter: string | undefined }> => {
...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  try {
    ...
    if (action === "add" && text) {
      ...
      console.log("TOGGLE");
     toggleTodo(parseInt(id)); // Cambia el estado de la tarea (completada o no)
    } else if (action === "delete" && id) {
      console.log("BORRAR")
      deleteTodo(parseInt(id)); // Elimina la tarea del array de tareas
